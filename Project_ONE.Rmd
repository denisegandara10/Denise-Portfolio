...
output: 
  pdf_document:
    ...
    toc: true
    toc_depth: 4
title: "Time Series Forecasting STD Dynamics in San Diego"
author: 
...
\section{Problem}
**The problem is to find which model most accurately predicts/forecasts future cases of Chlamydia and Gonorrhea in San Diego population.** <br>
...
We will use two different models, ARIMA and Exponential Smoothing method, and evaluate their performance using root mean square error and mean absolute error.
...
\subsection{Yearly Analysis-Boxplots} 
...
```
Calculated the total number of cases for each year.
...
#head(SD.df)
b = boxplot(Chlamydia ~ Year, data = SD.df, xlab = "Year", ylab = "Chlamydia cases", main = "Chlamydia Boxplots per Year", col = c("#bef7ff", "#c3f3ee", "#c8eede", "#cdeacd", "#d2e5bc", "#d7e1ac", "#dcdd9b", "#e1d88a", "#e6d479", "#ebd069", "#f0cb58", "#f5c747", "#fac237", "#ffbe26"))
...
c = boxplot(Gonorrhea ~ Year, data = SD.df, xlab = "Year", ylab = "Gonorrhea cases", main = "Gonorrhea Boxplots per Year", col = c("#bef7ff", "#bcf0ff", "#bae8ff", "#b8e1ff", "#b6d9ff", "#b4d2ff", "#b2cbff", "#b1c3ff", "#afbcff", "#adb5ff", "#abadff", "#a9a6ff", "#a79eff", "#a597ff"))
...
```{r}
d = acf(SD.df.train$Chlamydia)  # it looks exponential. Looks like S=3
                            #ACF doesn't go to zero, it needs differencing, remove trend. Use ARIMA
e = pacf(SD.df.train$Chlamydia) #try AR(3) S=3
...
```{r}
d = acf(SD.df.train$Gonorrhea) #looks exponential, S=3 maybe
e = pacf(SD.df.train$Gonorrhea) #try AR(3) S=3
plot_grid(d,e)
...
```{r}
#Chlamydia
e = autoplot(decompose(tsdata.C))

#Gonorrhea
f = autoplot(decompose(tsdata.G))
...
```{r}
g= ggseasonplot(tsdata.C) +
    ggtitle("Seasonal plot Chlamydia")

h =ggseasonplot(tsdata.G) +
...
print("ARIMA evaluation- Chlamydia")
accuracy(C.forecast, actual.C)
print("ETS evaluation- Chlamydia")
accuracy(frct, actual.C)

#Gonorrhea models comparison
print("ARIMA evaluation- Gonorrhea")
accuracy(G.forecast, actual.G)
print("ETS evaluation- Gonorrhea")
